...
from layers.ConvLSTMCell import ConvLSTMCell as convlstm
...
def rnn(images, mask_true, num_layers, num_hidden, filter_size, stride=1,
    ...
    for i in xrange(num_layers):
        ...
        new_cell = convlstm('lstm_'+str(i+1),
                          filter_size,
                          num_hidden_in,
                          num_hidden[i],
                          shape,
    ...
    for t in xrange(seq_length-1):
        ...
        with tf.variable_scope('predrnn', reuse=reuse):
            ...
            hidden[0], cell[0] = lstm[0](inputs, hidden[0], cell[0])

            for i in xrange(1, num_layers):
                hidden[i], cell[i] = lstm[i](hidden[i-1], hidden[i], cell[i])
