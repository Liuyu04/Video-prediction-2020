...
##### the spatial-temporal Conv-LSTM reproduce by liuyu at 2020/3/20 for comparison
...
class STLSTMCell():
    ...
    def __call__(self, x, h, c, m):
        ...
        with tf.variable_scope(self.layer_name):
            ...
            i_m, f_m, g_m = tf.split(m_cc, 3, 3)  ### check
            ...
            if x is None:
                ...
                gg = tf.tanh(g_m)  ### change by liuyu
            else:
                ii = tf.sigmoid(i_x_ + i_m)
                ff = tf.sigmoid(f_x_ + f_m + self._forget_bias)
                gg = tf.tanh(g_x_ + g_m)  ### change by liuyu
            ...
            if x is None:
                o = tf.sigmoid(o_c + o_m + o_h)  ###  change 3/10
            else:
                o = tf.sigmoid(o_x + o_c + o_m + o_h)  ### check 3/10
